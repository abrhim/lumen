...
- **F11 (disputed, optional):** Consider asserting the auth-client config (`flowType:"pkce", autoRefreshToken:false, detectSessionInUrl:false`) in the H1 stub. First verify the static refutation (that @supabase/ssr's createServerClient hardcodes these, making the app block documentary) — if confirmed, this is documentation-only, skip or downgrade. **[RESOLVED 2026-07-10: refutation CONFIRMED — @supabase/ssr@0.12.0 spreads the caller's `options?.auth` then hardcodes all three after it, so the library always wins; the app block is documentary. Skipped per the item's own terms.]**
